...
function showCheckboxes(index) {
	...
	if (!expanded[index - 1]) {
		closeOtherMenus(index - 1); // Schließen Sie andere Menüs
...
// Funktion zum Schließen aller Dropdown-Menüs außer dem angegebenen Index
...
// Event-Listener für Klickereignisse auf dem Dokument hinzufügen
document.addEventListener("click", function (event) {
	// Überprüfen Sie, ob der geklickte Bereich nicht Teil des Dropdown-Menüs ist
	if (!event.target.closest('.multiselect')) {
		// Schließen Sie alle geöffneten Dropdown-Menüs
		for (let i = 0; i < expanded.length; i++) {
			if (expanded[i]) {
				showCheckboxes(i + 1);
			}
